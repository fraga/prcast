...
script = [
  {
    "speaker": "Andrew",
    "text": "Welcome back to PRCast. Today we're looking at Pull Request 5167 in the unified operations public repo."
  },
  {
    "speaker": "Sarah",
    "text": "That's right, Andrew. This one is from Milda Beinaryte, updating the bank revaluation documentation with some new enhancements."
  },
  {
    "speaker": "Andrew",
    "text": "Ah, foreign currency revaluation. Always a hot topic. What's new?"
  },
  {
    "speaker": "Sarah",
    "text": "The update adds details about enhancements to the feature, specifically around the setup requirements and some troubleshooting steps."
  },
  {
    "speaker": "Andrew",
    "text": "It looks like a focused update, just one file changed, adding 21 lines. That should definitely help folks configuring their bank foreign currency revaluations."
  },
  {
    "speaker": "Sarah",
    "text": "Exactly. Better setup and troubleshooting docs mean fewer headaches when dealing with multiple currencies. That's all for today, thanks for listening!"
  }
]
...
async def main():
    ...
    for i, line in enumerate(script):
        filename = f"/tmp/line_{i}.mp3"
        await generate_line(line["text"], VOICES[line["speaker"]], filename)
        files.append(filename)

    print("Stitching voice audio...")
    voice_audio = AudioSegment.empty()
    pause = AudioSegment.silent(duration=400)
    for f in files:
        voice_audio += AudioSegment.from_mp3(f) + pause
        os.remove(f)

    print("Mixing with intro and outro...")
    intro = AudioSegment.from_mp3(base / "intro.mp3")
    outro = AudioSegment.from_mp3(base / "outro.mp3")

    # Let the intro play full volume for 6 seconds, then crossfade the voice in
    # while gradually fading the background track down over 4 seconds.
    intro_lead = intro[:6000]
    
    # We take the rest of the intro, lower its base volume by 20dB so it sits behind the speech,
    # and we add a 4-second fade-in to that drop so it's a smooth slope down, not a cliff.
    # Note: pydub doesn't have a native 'duck' curve, so we apply the volume drop
    # and use overlay to blend it.
    intro_tail = intro[6000:] - 18
    
    # Crossfade the voice onto the tail of the intro
    # The voice starts right at 6000ms
    combined = intro_lead + intro_tail.overlay(voice_audio)
    
    # If the voice is longer than the intro tail, append the rest of the voice
    if len(voice_audio) > len(intro_tail):
        remaining_voice = voice_audio[len(intro_tail):]
        combined += remaining_voice
        
    # Append the outro, crossfading it with the very end of the voice, softer fade
    combined = combined.append(outro, crossfade=4000)
    ...
    title='PR #5167: Update bank revaluation documentation with enhancements (With Music!)'
